...
fnet <- function(x,Simplify=FALSE,threshold=0.05,
                 max.ew=2,max.size=18,node=TRUE,
  ...
  xc$rfreq = xc$freq/n
  xc$size=max.size+(1-xc$rfreq)*10
  ...
  if(Simplify || threshold !=0){
    ...
    xc = xc[match(unique_name,xc$name),]
  }
  ...
  igraph::V(x_nets)$size <- xc$size[match(igraph::V(x_nets)$name,xc$name)]
  ...
  if(print.graph){
    plot(x_nets,
         vertex.size=ifelse(node,igraph::V(x_nets)$size,0),
         edge.width=ew,...) 
  }
